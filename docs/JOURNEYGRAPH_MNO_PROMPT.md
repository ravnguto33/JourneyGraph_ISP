...
- **Praça**: **Brasília (DF)**, como segunda cidade da arquitetura multi-cidade (a primeira, Florianópolis, é a do JourneyGraph ISP atual — fixo, não móvel);
...
### 1.2 Por que Brasília e por que Claro

São Paulo foi a cidade-alvo inicial da arquitetura multi-cidade (`cidade_alvo_SP.yaml`, já validada nesta mesma execução — dados em `dados_sp/`), mas a praça foi trocada para **Brasília (DF)** a pedido do usuário. `C:\PERSDEV\poc_personas_v2\pipeline\cidade_alvo_BSB.yaml` foi criada com o gerador do próprio pipeline (`gerar_config_cidade.py`), código IBGE 5300108, e validada com `cidade_loader.py --check`. Claro é o escopo inicial de operadora — mas **operadora é parâmetro do pipeline de antenas, não hardcode**, para permitir trocar para TIM/Vivo/Oi sem reescrever código (ver §3.2 e §4 — essa parametrização foi implementada como parte deste trabalho, ver `00_preparar_antenas_anatel.py`/`01_gerar_base_mobilidade.py`).
...
| Cidade | Única, hardcoded (Florianópolis) | Multi-cidade parametrizada via `cidade_alvo_XXX.yaml` (§4) — Brasília (DF) como segunda praça, mesma arquitetura reaproveitável para novas cidades (São Paulo já validada como praça intermediária, dados em `dados_sp/`) |
...
- **Wave 2 (ATIVO — implementado em 2026-08-05, adaptado sem CRM/Billing)**: P5 Big Data User, P6 Usuário de Voz Premium, P7 Usuário Multi-SIM/Itinerante, P8 Usuário Social/Comunicação Intenso. Os critérios de inclusão/exclusão originais da curadoria dependiam de dado de CRM (percentis relativos a outras personas, plano contratado) que a base sintética CDR-only não produz — adaptados para usar só features derivadas do CDR (`pct_noite_medio`, `voz_dur_media_min`, `n_dias_ativos`, `cv_sessoes_dia`, adicionadas ao script 04), mantendo a intenção discriminante de cada persona. K-Means K=8 + XGBoost: 84,2% de acurácia em 8 classes.
...
Ao ampliar a planilha para o cenário MNO/Brasília, seguir a mesma estrutura de colunas e o mesmo padrão de `criterio_inclusao`/`criterio_exclusao` formal (não texto livre) usado nas 8 personas já curadas.
...
**O que é específico por cidade**: os valores numéricos acima (bbox, população, `n_clusters` — 27 para RM Florianópolis, 50 para o município de SP, 35 para Brasília/DF — área menor que SP mas com regiões administrativas espalhadas, daí `raio_max_km` mais alto que o default), a lista de `clusters_referencia` (nomes de bairros/regiões relevantes que precisam de renomeação manual pós-K-Means) e os `eventos_recorrentes` locais (não há automação para isso — precisa ser preenchido por quem conhece a cidade, mesma ressalva que o README do `poc_personas_v2` já registra; `cidade_alvo_BSB.yaml` foi gerada via CLI direta, sem eventos curados ainda — item aberto se o produto precisar de sazonalidade local de Brasília).

`config/cidade_alvo_BSB.yaml` foi criada nesta execução (código IBGE 5300108, UF DF, bounding box do Distrito Federal, `--n-clusters 35 --raio-max-km 8.0`), com `antenas_bsb_claro.csv` gerado a partir do catálogo real da Anatel — 4.186 antenas Claro dentro da bounding box do DF (bem menos que as 29.824 de SP, coerente com a área ~5x menor). O trabalho deste prompt não recria a infraestrutura de cidade — consome o `ConfigCidade` de Brasília e a integra ao `RAW` do `journeygraph-mno/index.html`, incluindo a parametrização de MNO do §3.2 (eixo ortogonal à cidade: bbox/IBGE filtram geografia, operadora filtra prestadora).
...
- [ ] Antenas Anatel de Brasília (DF), filtradas por operadora parametrizável (não hardcoded Claro no código do app nem do pipeline), com schema §3.2 incluindo o campo `operadora`.
...
- [ ] Configuração de cidade via `cidade_alvo_BSB.yaml` consumida corretamente (bbox, mapa, eventos, calendário, clima) e refletida no `RAW.cidade`.
...
- **Outras operadoras além de Claro na primeira entrega de dados** — o pipeline aceita `--mno` como parâmetro (§3.2/§6), mas o dataset gerado nesta entrega é Claro/Brasília. TIM/Vivo/Oi ficam para quando houver necessidade de demonstração multi-operadora — e seus MNC/CNPJ de referência em `OPERADORAS`/`MCC_MNC_POR_OPERADORA` ainda precisam de confirmação oficial antes de uso além de demonstração (ver comentários nos scripts 00/01).
- ~~Personas Wave 2 (P5–P8)~~ — implementadas em 2026-08-05 (adaptadas sem CRM/Billing, ver §3.3). Item removido do escopo de fora.
